...
+ (NSString *)rs_generateURLWithPattern:(NSString *)pattern parameters:(NSArray *)parameters {
    return [[self class] rs_generateURLWithPattern:pattern parameters:parameters extraParameters:nil];
}
...
+ (NSString *)rs_generateURLWithPattern:(NSString *)pattern parameters:(NSArray *)parameters extraParameters:(NSDictionary *)extraParameters {
    ...
    if (routeValues && routeValues.count) {
        ...
        return [NSString stringWithFormat:@"/%@%@", urlPath, [self rs_mapDictionaryToURLQueryString:extraParameters]];
    }
    return nil;
}
